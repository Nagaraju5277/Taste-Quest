  ...
  return (
    <>
      <nav className='flex justify-between items-center gap-4 bg-slate-800 lg:px-10  px-4 py-6 sticky top-0 z-30'>
        <div id='logo'>
          <h1 className='lg:text-[35px] text-[28px] text-orange-500 font-bold '>Taste Quest</h1>

        </div>
        <ul className='lg:flex justify-center items-center gap-10 hidden'>
          {navItems.map(({ link, path }) => (
            <Link key={path} to={path} spy={true} offset={-100} smooth={true} className='text-white uppercase font-semibold cursor-pointer p-3 rounded-lg hover:bg-orange-500 hover:text-black'>
              {link}
            </Link>
          ))}
        </ul>
      ...
      <button className='bg-green-500 px-8 py-3 rounded-full hover:bg-orange-500 hover:text-black font-bold  hidden lg:flex'>
ORDER NOW
      </button>
      </nav>
